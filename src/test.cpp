...
template <typename Game, typename Agent1, typename Agent2>
class GameSession {
        public:
...
int main(int argc, char* argv[])  {
        ...
        GameSession<TicTacToe, PickRandomActionAgent<TicTacToe>, PickRandomActionAgent<TicTacToe>> session;
        high_resolution_clock::time_point t1 = high_resolution_clock::now();
        for(int i = 0; i < num_games; i++) {
                switch(session.PlayOnce()) {
                        case GameStatus::X_WINS: x_wins++; break;
                        case GameStatus::O_WINS: o_wins++; break;
                        case GameStatus::DRAW: draws++; break;
                }
                session.Reset();
        }
